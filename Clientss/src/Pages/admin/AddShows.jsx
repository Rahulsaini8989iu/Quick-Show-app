...
const AddShows = () => {
  ...
  const fetchNowPalyingMovies = async () => {
                ...
                <div className="flex flex-wrap gap-2 mt-1 text-sm">
                  {times.map((time) => (
                    <div key={time} className="border border-primary px-2 py-1 flex items-center rounded">
                      <span>{time}</span>
                      <DeleteIcon className='text-red-500 ml-2 cursor-pointer hover:text-red-700' onClick={() => handleRemoveTime(date, time)} />
                    </div>
                  ))}
                </div>
              </li>))}
